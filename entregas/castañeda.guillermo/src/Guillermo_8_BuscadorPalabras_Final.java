    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            for (int i = 0; i < diccionario.length; i++) {
                ...
                if (diccionario[i].length() < limite) {
                    limite = diccionario[i].length();
                }

                for (int j = 0; j < limite; j++) {
                    if (palabra.substring(j, j + 1).equals(diccionario[i].substring(j, j + 1))) {
                        coincidencias++;
                    }
                }
                if (coincidencias >= 3) {
            ...
            for (int i = 0; i < diccionario.length; i++) {
                ...
                for (int j = 0; j <= diccionario[i].length() - palabra.length(); j++) {
                    ...
                    for (int k = 0; k < palabra.length(); k++) {
                        if (!diccionario[i].substring(j + k, j + k + 1).equals(palabra.substring(k, k + 1))) {
                            coincide = false;
                        }
                    }
                    if (coincide) {
