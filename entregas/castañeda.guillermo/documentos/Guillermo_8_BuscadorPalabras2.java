...
public class BuscadorPalabras {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            for (int i = 0; i < diccionario.length; i++) {
                int c = 0;
                int limite = Math.min(p.length(), diccionario[i].length());
                for (int j = 0; j < limite; j++) {
                    if (p.charAt(j) == diccionario[i].charAt(j)) {
                        c++;
                    }
                }
                if (c >= 3) {
            ...
            for (int i = 0; i < diccionario.length; i++) {
                boolean f = false;
                for (int j = 0; j <= diccionario[i].length() - p.length(); j++) {
                    boolean m = true;
                    for (int k = 0; k < p.length(); k++) {
                        if (diccionario[i].charAt(j + k) != p.charAt(k)) {
                            m = false;
                        }
                    }
                    if (m) {
